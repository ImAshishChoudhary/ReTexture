...
async def remove_background(file: UploadFile = File(...)):
    ...
    try:
        ...
        print(f"[AGENT DEBUG] Original image size: {original_size}, mode: {original_mode}")
        
        # Always resize to be safe - rembg works best with smaller images
        if max(img.size) > MAX_SIZE:
            print(f"[AGENT DEBUG] Image too large ({max(img.size)}px), resizing to max {MAX_SIZE}px...")
        ...
        save_format = 'PNG' if img.mode == 'RGBA' else 'PNG'  # Always use PNG for quality
        ...
        print(f"[AGENT DEBUG] Base64 encoding complete! Length: {len(base64_image)} chars")
...
async def generate_variations(req: VariationsRequest):
    ...
    try:
        ...
        try:
            ...
            print(f"[AGENT DEBUG] Calling generate_variations_from_bytes with concept: {req.concept}")
            ...
            if variations:
                ...
                for i, v in enumerate(variations):
                    print(f"[AGENT DEBUG] Variation {i+1} length: {len(v) if v else 0} chars")
            ...
            print(f"[AGENT DEBUG] AI generation error: {type(ai_error).__name__}: {ai_error}")
            import traceback
    ...
    async def event_generator():
        ...
        try:
            ...
            for i, style in enumerate(styles):
                ...
                try:
                    print(f"[AGENT] 🎨 Calling generate_single_variation for {style}...")
                    variation = generate_single_variation(image_bytes, concept, style)
                    
                    if variation:
                        print(f"[AGENT] ✅ Variation {i+1} generated! Length: {len(variation)}")
                        variation_event = f"data: {json.dumps({'type': 'variation', 'index': i, 'data': variation})}\n\n"
                        print(f"[AGENT] 📤 Sending VARIATION event (length: {len(variation_event)})")
