...
export function checkBlockedKeywords(elements) {
  ...
  textElements.forEach((el, index) => {
    ...
    console.log(`  🔍 Checking text ${index + 1}/${textElements.length}: ${el.id}`);
  ...
  console.log(`✅ checkBlockedKeywords complete: ${violations.length} violations`);
  return violations;
}
...
export function checkRequiredElements(elements) {
  ...
  const hasHeadline = textElements.some(el => {
    ...
    if (isHeadline && !isTag) {
      console.log(`  ✅ Found headline: "${text.substring(0, 30)}..." (${fontSize}px)`);
      return true;
    }
    return false;
  });
  ...
  console.log(`✅ checkRequiredElements complete: ${violations.length} violations`);
  return violations;
}
...
export function checkClubcardDate(elements) {
  ...
  clubcardElements.forEach((el, index) => {
    const text = el.text || '';
    console.log(`  🔍 Checking Clubcard ${index + 1}/${clubcardElements.length}: ${el.id}`);
    console.log(`    ↳ Content: "${text}"`);
