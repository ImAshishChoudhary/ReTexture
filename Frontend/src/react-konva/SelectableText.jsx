...
export default function SelectableText({
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === "Backspace") {
        if (cursorPosition > 0) {
          newText =
            draftText.slice(0, cursorPosition - 1) +
        ...
        if (cursorPosition < draftText.length) {
          newText =
            draftText.slice(0, cursorPosition) +
            draftText.slice(cursorPosition + 1);
        }
  ...
  const getCursorX = () => {
    ...
    context.font = `${shape?.bold ? "bold " : ""}${
      shape?.italic ? "italic " : ""
    }${shape?.fontSize || 16}px ${shape?.fontFamily || "Arial"}`;
  ...
  useEffect(() => {
    if (ref.current && shape?.textTransform) {
      setDraftText((text) => {
        if (shape?.textTransform === "lowercase")
