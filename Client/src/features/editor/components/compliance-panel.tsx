...
import { CheckCircle2, XCircle, AlertTriangle, Loader2, Wand2, ChevronDown } from "lucide-react";
import { validateCanvas, generateContentForFix, type CanvasObject } from "@/lib/api-client";
...
const RULES: ComplianceRule[] = [
  { id: "TESCO_TAG", name: "Tesco Tag", desc: "Tag sticker required", cat: "Essential", fix: true },
  { id: "HEADLINE", name: "Headline", desc: "Headline required", cat: "Essential", fix: true },
  { id: "SUBHEAD", name: "Subhead", desc: "Subheading recommended", cat: "Essential", fix: true },
  { id: "LOGO", name: "Logo", desc: "Brand logo required", cat: "Essential", fix: true },
  { id: "PACKSHOT", name: "Packshot", desc: "Product image required", cat: "Essential", fix: false },
  { id: "MIN_FONT_SIZE", name: "Font Size", desc: "Min 20px", cat: "Access", fix: true },
  { id: "CONTRAST", name: "Contrast", desc: "4.5:1 ratio", cat: "Access", fix: false },
  { id: "SAFE_ZONE", name: "Safe Zone", desc: "No text in zones", cat: "Layout", fix: true },
  { id: "TEXT_ALIGNMENT", name: "Alignment", desc: "Consistent align", cat: "Layout", fix: true },
  { id: "TAG_OVERLAP", name: "Tag Overlap", desc: "Tag clear", cat: "Layout", fix: true },
  { id: "PACKSHOT_GAP", name: "Packshot Gap", desc: "Min 24px gap", cat: "Layout", fix: true },
  { id: "BLOCKED_COPY", name: "Blocked Words", desc: "No restricted", cat: "Content", fix: false },
  { id: "NO_CTA", name: "No CTA", desc: "CTAs not allowed", cat: "Content", fix: true },
  { id: "TAG_TEXT", name: "Tag Text", desc: "Valid tag text", cat: "Content", fix: true },
  { id: "CLUBCARD_DATE", name: "Clubcard Date", desc: "Date required", cat: "Content", fix: false },
  { id: "DRINKAWARE", name: "Drinkaware", desc: "For alcohol", cat: "Content", fix: false },
  { id: "VALUE_TILE_OVERLAP", name: "Value Tile", desc: "No overlap", cat: "Visual", fix: false },
  { id: "PEOPLE_PHOTO", name: "People Photo", desc: "Not recommended", cat: "Visual", fix: false },
  { id: "BACKGROUND", name: "Background", desc: "BG required", cat: "Visual", fix: true },
  { id: "LEP_BACKGROUND", name: "LEP BG", desc: "LEP rules", cat: "Visual", fix: false },
];
...
const loadImg = (src: string): Promise<fabric.Image> => {
  return new Promise((resolve, reject) => {
    fabric.Image.fromURL(src, (img) => img ? resolve(img) : reject(new Error("Failed")), { crossOrigin: "anonymous" });
  });
};
...
export function CompliancePanel({ editor, canvasWidth, canvasHeight }: Props) {
  ...
  const getCanvasObjects = useCallback((): CanvasObject[] => {
    ...
    return canvas.getObjects().map((obj: fabric.Object, i: number) => ({
      ...
      text: ["textbox", "i-text", "text"].includes(obj.type || "") ? (obj as fabric.Textbox).text : undefined,
      fontSize: ["textbox", "i-text", "text"].includes(obj.type || "") ? (obj as fabric.Textbox).fontSize : undefined,
  ...
  // Generate AI content
  const generateContent = useCallback(async (rule: string, ctx?: string): Promise<string> => {
    try {
      const res = await generateContentForFix({
        rule,
        context: ctx,
        canvas_objects: getCanvasObjects().map(o => ({ type: o.type, text: o.text, fontSize: o.fontSize })),
      });
  ...
  const validateMutation = useMutation({
    mutationFn: async () => validateCanvas({ width: canvasWidth, height: canvasHeight, objects: getCanvasObjects() }),
    onSuccess: (res) => {
      if (res.data) {
        const mapped = RULES.map((rule) => {
          ...
          if (fail) return { rule, status: "fail" as const, message: fail.message };
          if (warn) return { rule, status: "warning" as const, message: warn.message };
  ...
  const handleFix = useCallback(async (ruleId: string) => {
    ...
    try {
      ...
      const findEmptySpace = (width: number, height: number, preferTop: boolean = true): { x: number; y: number } => {
        const padding = 20;
        ...
        const positions = preferTop ? [
          ...
          { x: canvasWidth - width - padding, y: canvasHeight - height - padding }, // bottom-right
        ];
        ...
        return preferTop ? { x: padding, y: padding } : { x: padding, y: canvasHeight - height - padding };
      };
      
      switch (ruleId) {
        case "TESCO_TAG": {
          // Check if Tesco tag already exists
          const existingTag = objects.find((o: fabric.Object) => 
            (o as any).isTescoTag || 
        ...
        case "LOGO": {
          ...
          const existingLogo = objects.find((o: fabric.Object) => (o as any).isLogo || (o as any).customId?.includes("logo"));
          if (existingLogo) {
        ...
        case "PACKSHOT_GAP": {
          ...
          const packshots = objects.filter((o: fabric.Object) => 
            o.type === "image" && 
          ...
          for (let i = 1; i < packshots.length; i++) {
            ...
            if (currentGap < minGap) {
              ...
              if (newLeft + (curr.width || 0) * (curr.scaleX || 1) < canvasWidth - 20) {
          ...
          for (let i = 1; i < packshots.length; i++) {
            ...
            if (currentGap < minGap && currentGap > -50) {
              ...
              if (newTop + (curr.height || 0) * (curr.scaleY || 1) < canvasHeight - 20) {
                curr.set("top", newTop);
                adjusted++;
              }
            }
        ...
        case "HEADLINE": {
          ...
          const texts = objects.filter((o: fabric.Object) => ["textbox", "i-text", "text"].includes(o.type || ""));
          const hasHeadline = texts.some((t: fabric.Object) => ((t as fabric.Textbox).fontSize || 0) >= 36);
          if (hasHeadline) {
        ...
        case "SUBHEAD": {
          const hasSubhead = objects.some((o: fabric.Object) => (o as any).customId === "subhead");
          if (hasSubhead) {
          ...
          const headline = objects.find((o: fabric.Object) => 
            (o as any).customId === "headline" || 
            (["textbox", "i-text", "text"].includes(o.type || "") && ((o as fabric.Textbox).fontSize || 0) >= 36)
          );
          ...
          if (headline) {
            ...
            posY = (headline.top || 0) + (headline.height || 50) * (headline.scaleY || 1) + 15;
          } else {
        ...
        case "TAG_OVERLAP": {
          const tags = objects.filter((o: fabric.Object) => (o as any).isTescoTag || (o as any).customId?.includes("tesco"));
          if (tags.length === 0) {
        ...
        case "SAFE_ZONE": {
          ...
          objects.forEach((o: fabric.Object) => {
            if (["textbox", "i-text", "text"].includes(o.type || "")) {
              ...
              const objBottom = (o.top || 0) + (o.height || 0) * (o.scaleY || 1);
              if (objBottom > canvasHeight - safeMargin) {
                o.set("top", canvasHeight - safeMargin - (o.height || 0) * (o.scaleY || 1) - 10);
                moved++;
              }
            }
          });
        ...
        case "NO_CTA": {
          const ctaPattern = /\b(shop now|buy now|click here|order now|add to cart|learn more)\b/gi;
          let removed = 0;
        ...
        case "BACKGROUND": {
          const hasBg = objects.some((o: fabric.Object) => (o as any).isBackground);
          if (hasBg) {
  ...
  const grouped = useMemo(() => {
    ...
    CATS.forEach(c => {
      g[c] = validated ? results.filter(r => r.rule.cat === c) : RULES.filter(r => r.cat === c).map(rule => ({ rule, status: "pending" as const }));
    });
    return g;
  }, [results, validated]);
    ...
    <div className="space-y-3">
      ...
      {validated && (
        <div className="flex gap-3 p-2 rounded-lg bg-neutral-800/50 text-xs">
          <span className="flex items-center gap-1 text-green-400"><CheckCircle2 className="w-3 h-3" />{stats.pass}</span>
          <span className="flex items-center gap-1 text-red-400"><XCircle className="w-3 h-3" />{stats.fail}</span>
          <span className="flex items-center gap-1 text-yellow-400"><AlertTriangle className="w-3 h-3" />{stats.warn}</span>
        </div>
      )}
      ...
      >
        {validateMutation.isPending ? <><Loader2 className="w-4 h-4 animate-spin" />Validating...</> : "Validate Canvas"}
      </button>
        ...
        {CATS.map(cat => {
          ...
          return (
            <div key={cat} className="rounded-lg border border-white/10 overflow-hidden">
              <button onClick={() => toggle(cat)} className="w-full flex items-center justify-between px-3 py-2 bg-neutral-800/40 hover:bg-neutral-800/60 text-xs">
                <span className="font-medium text-neutral-200">{cat}</span>
                <div className="flex items-center gap-1.5">
                  {fails > 0 && <span className="px-1.5 py-0.5 bg-red-500/20 text-red-400 rounded text-[10px]">{fails}</span>}
                  {warns > 0 && <span className="px-1.5 py-0.5 bg-yellow-500/20 text-yellow-400 rounded text-[10px]">{warns}</span>}
              ...
              {open && (
                <div className="divide-y divide-white/5">
                  {items.map(({ rule, status, message }) => (
                    <div key={rule.id} className={cn("px-3 py-2", status === "fail" && "bg-red-500/5", status === "warning" && "bg-yellow-500/5")}>
                      <div className="flex items-center gap-2">
                        <div className="shrink-0">
                          {status === "pass" && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                          {status === "fail" && <XCircle className="w-3.5 h-3.5 text-red-500" />}
                          {status === "warning" && <AlertTriangle className="w-3.5 h-3.5 text-yellow-500" />}
                          {status === "pending" && <div className="w-3.5 h-3.5 rounded-full border border-neutral-600" />}
                        </div>
                        <span className="flex-1 text-xs font-medium text-neutral-200">{rule.name}</span>
                        {(status === "fail" || status === "warning") && rule.fix && (
                          <button
                          ...
                          >
                            {fixing === rule.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <><Wand2 className="w-3 h-3" />Fix</>}
                          </button>
                        )}
                      </div>
                      <p className="text-[10px] text-neutral-500 mt-0.5 ml-5 line-clamp-1">{message || rule.desc}</p>
                    </div>
                  ))}
                </div>
              )}
