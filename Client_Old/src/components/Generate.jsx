...
export default function Generate({ setPagesWithHistory }) {
  const { activeIndex, canvasSize, editorPages, selectedUniqueId } =
    useEditorStore();
  ...
  const firstImageOnCanvas = activePage?.children?.find(
    (el) => el.type === "image"
  );

  // Detect logo position for smart placement
  const detectedLogoPosition = (() => {
    const logo = activePage?.children?.find(
      (el) => el.id && el.id.startsWith("logo-")
    );
  ...
  // Calculate image bounds for TescoLogo-style placement with PROTECTION BUFFER
  const PRODUCT_PROTECTION_BUFFER = 30; // Match backend buffer
  const imageBounds = (() => {
    const mainImage = activePage?.children?.find(
      (el) => el.type === "image" && !el.id?.startsWith("logo-")
    );
    if (mainImage) {
      ...
      console.log("📐 [GENERATE] Calculated Product Bounds:", bounds);
      console.log(
        "📦 [GENERATE] Protection Buffer:",
        PRODUCT_PROTECTION_BUFFER,
        "px"
      );
      return bounds;
    }
  ...
  const getCanvasImage = useCallback(async () => {
    ...
    const imageElement = activePage?.children?.find(
      (el) => el.type === "image"
    );
    if (imageElement?.src) {
      try {
  ...
  useEffect(() => {
    if (activePage?.children) {
      console.log(
        "🖼️ [GENERATE] Active page updated. Item count:",
        activePage.children.length
      );
      const lastItem = activePage.children[activePage.children.length - 1];
      if (lastItem) {
        console.log(
          "🖼️ [GENERATE] Last item added:",
          lastItem.type,
          lastItem.text || lastItem.id
        );
      }
    }
  }, [activePage]);
  ...
  const handleAddHeadline = useCallback(
    (text, position) => {
      ...
      setPagesWithHistory((prev) => {
        ...
        const pageIndex =
          activeIndex >= 0 && activeIndex < cp.length ? activeIndex : 0;

        if (!cp[pageIndex]) {
          console.error(
            "❌ [GENERATE] Active page index invalid:",
            activeIndex
          );
          return prev;
        }
        ...
        const existingHeadlines = cp[pageIndex].children.filter((el) =>
          el.id?.startsWith("headline-")
        );
        if (existingHeadlines.length > 0) {
          ...
          cp[pageIndex].children = cp[pageIndex].children.filter(
        ...
        console.log(
          `✅ [GENERATE] Added headline "${text.substring(
            0,
            10
          )}..." to page ${pageIndex}`
        );
  ...
  const handleAddSubheading = useCallback(
    (text, position) => {
      ...
      // Use bold property (SelectableText uses bold/italic booleans, not fontStyle)
      const isBold =
        position?.fontWeight >= 600 || position?.fontWeight === "bold";
      ...
      setPagesWithHistory((prev) => {
        ...
        const existingSubheadings = cp[pageIndex].children.filter((el) =>
          el.id?.startsWith("subheading-")
        );
        if (existingSubheadings.length > 0) {
          console.log(
            "🗑️ [GENERATE] Removing",
            existingSubheadings.length,
            "existing subheading(s)"
          );
          cp[pageIndex].children = cp[pageIndex].children.filter(
        ...
        cp[pageIndex].children.push(newTextElement);
        console.log(
          `✅ [GENERATE] Added subheading "${text.substring(
            0,
            10
          )}..." to page ${pageIndex}`
        );
  ...
  const handleRemoveBackground = async () => {
    ...
    console.log(
      "[FRONTEND DEBUG] Selected element src exists:",
      !!selectedEl?.src
    );
    ...
    try {
      ...
      console.log(
        "[FRONTEND DEBUG] Original image src type:",
      ...
      if (imageSrc.startsWith("blob:") || imageSrc.startsWith("http")) {
        ...
        console.log(
          "[FRONTEND DEBUG] Conversion complete, new src type:",
          imageSrc.substring(0, 30) + "..."
        );
      }
      ...
      if (!imageSrc.startsWith("data:")) {
        console.log(
          "[FRONTEND DEBUG] ERROR: Invalid image format after conversion"
        );
      ...
      console.log(
        "[FRONTEND DEBUG] Blob created, size:",
        blob.size,
        "type:",
        blob.type
      );
      ...
      console.log(
        "[FRONTEND DEBUG] Calling Agent API:",
        `${AGENT_URL}/remove-bg`
      );
      ...
      console.log(
        "[FRONTEND DEBUG] - image_data length:",
      ...
      if (!result.success || !result.image_data) {
        ...
        console.log(
          "[FRONTEND DEBUG] - result image_data length:",
      ...
      console.log(
        "[FRONTEND DEBUG] handleRemoveBackground finished, setting processing=false"
      );
      setProcessing(false);
    }
  };
  ...
  const handleGenerateVariations = async () => {
    ...
    try {
      ...
      console.log(
        "[FRONTEND] Background removed! image_data length:",
        bgResult.image_data?.length
      );
      ...
      message.loading({
        content: "🎨 Generating variations (streaming)...",
        key: "var",
        duration: 0,
      });
      ...
      console.log(
        "[FRONTEND] 📡 Starting to read SSE stream with buffering..."
      );
      ...
      while (true) {
        ...
        chunkCount++;
        console.log(
          `[FRONTEND] 📦 Chunk ${chunkCount}: done=${done}, bytes=${
            value?.length || 0
          }`
        );
        ...
        console.log(
          `[FRONTEND] Processing ${events.length} complete events, buffer remaining: ${buffer.length} chars`
        );
        ...
        for (const event of events) {
          ...
          for (const line of lines) {
            if (line.startsWith("data: ")) {
              ...
              console.log(
                `[FRONTEND] 📨 Parsing SSE data (${jsonStr.length} chars)`
              );
              ...
              try {
                ...
                if (data.type === "variation" && data.data) {
                  console.log(
                    `[FRONTEND] ✅ VARIATION ${
                      data.index + 1
                    } RECEIVED! Data: ${data.data.length} chars`
                  );
                  const newVariation = {
                    id: `var${Date.now()}_${data.index}`,
                    url: `data:image/png;base64,${data.data}`,
                    name:
                      styleNames[data.index] || `Variation ${data.index + 1}`,
                  };
                  ...
                  setVariations((prev) => {
                    console.log(
                      `[FRONTEND] 📊 Adding variation ${
                        data.index + 1
                      }, prev count: ${prev.length}`
                    );
                ...
                if (data.type === "error") {
                  console.log(
                    `[FRONTEND] ❌ Error for variation ${data.index}:`,
                    data.message
                  );
                ...
                if (data.type === "complete") {
                  ...
                  message.success({
                    content: "✨ All variations generated!",
                    key: "var",
                    duration: 3,
                  });
                }
      ...
      message.error({
        content: error.message || "Failed to generate",
        key: "var",
        duration: 4,
      });
    } finally {
      console.log(
        "[FRONTEND] 🏁 Generation complete, setting generatingVariations=false"
      );
  ...
  return (
    <>
          ...
          <Flex wrap="wrap" justify="center" gap={8}>
            ...
            {variations.map((v) => (
              ...
              >
                ...
                />
                <p style={{ fontSize: 10, margin: "4px 0 0", color: "#666" }}>
                  {v.name}
                </p>
              </div>
            ))}
      ...
      <HeadlineGenerator
        ...
        canvasElements={editorPages[activeIndex]?.children || []}
